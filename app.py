...
# Function to randomly highlight words
def random_highlight(text):
    words = text.split()  # Split the text into words
    highlighted_words = []
    
    for word in words:
        # Randomly decide if the word should be highlighted
        if random.random() < 0.3:  # 30% chance to highlight
            highlighted_word = f"<span style='background-color: yellow;'>{word}</span>"
        else:
            highlighted_word = word
        highlighted_words.append(highlighted_word)
    
    # Reassemble the text with spaces between words
...
if submit_button:
    if user_input.strip() == "":
        ...
        highlighted_input = random_highlight(user_input)

        
        if language == 'Arabic':
            st.markdown(f"<div style='direction: rtl; text-align: right;'>{highlighted_input}</div>", unsafe_allow_html=True)
        else:
            st.markdown(f"<div style='direction: ltr; text-align: left;'>{highlighted_input}</div>", unsafe_allow_html=True)
